...
static inline void soc_init(void) {
  ...
  REG(C3_SYSTEM)[22] = BIT(19) | (40U << 12) | BIT(10);
   asm volatile("csrw   mstatus, 0x8"); //mie a 1 para habilitar isr global
  // REG(C3_RTCCNTL)[47] = 0; // RTC_APB_FREQ_REG -> freq >> 12
  ((void (*)(int)) 0x40000588)(160);  // ets_update_cpu_frequency(160)
  uintptr_t _vector_address = (uintptr_t)&_vector_table;
  //asm volatile("la t0, %0" ::"r"(_vector_table));
  printf("vector_table value: %p \n", _vector_table);
  //asm volatile("ori %1, %0, 1" :"=r"(_vector_address) :"r"(_vector_address));
  asm volatile("csrrw zero, mtvec, %0" ::"r"(_vector_address));
  asm volatile ("csrr %0, mtvec" : "=r"(csr_mtvec));
  printf("mtvec value: %lu \n", csr_mtvec);
#if 0
